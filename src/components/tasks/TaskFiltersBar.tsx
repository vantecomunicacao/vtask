...
export function TaskFiltersBar({
    ...
    return (
        <div className="px-4 py-3 border-b border-border-subtle flex items-center gap-3 bg-surface-2/50 shrink-0 overflow-x-auto">
                ...
                {(['status', 'project', 'due_date'] as const).map(option => (
                    <button
                        ...
                        className={cn(
                            ...
                            groupBy === option
                                ? 'bg-surface-card text-brand border border-border-subtle'
            ...
            <div className="shrink-0">
                <button
                    ...
                    onClick={() => {
                        ...
                        if (rect) setFiltersPos({ top: rect.bottom + 4, left: rect.left });
                        setIsFiltersOpen(v => !v);
                        setIsViewOpen(false);
                    }}
                    className={cn(
                        "flex items-center gap-1.5 px-3 py-1.5 rounded-[var(--radius-md)] text-xs font-bold border transition-all duration-200",
                        activeFilterCount > 0
            ...
            <div className="shrink-0">
                <button
                    ...
                    onClick={() => {
                        ...
                        if (rect) setViewPos({ top: rect.bottom + 4, left: rect.left });
                        setIsViewOpen(v => !v);
                        setIsFiltersOpen(false);
                    }}
                    className={cn(
            ...
            {/* Portals — renderizados no body para escapar do overflow-x-auto */}
